...
print(f"{'STRATEGY':<22}{'AVG PTS':>9}{'RANK':>7}{'WINS':>7}{'BEST':>8}{'WORST':>8}")
print("-"*63)
res={}
for name,s in STRATS.items():
    ...
    for seed in range(10):
        me,alls=run(seed,s)
        tots.append(me); r=sorted(alls,reverse=True).index(me)+1
        ranks.append(r); wins+= (r==1)
    res[name]=(statistics.mean(tots),statistics.mean(ranks),wins,max(tots),min(tots))
for name,(a,r,w,b,wo) in sorted(res.items(),key=lambda x:-x[1][0]):
    print(f"{name:<22}{a:>9.0f}{r:>7.1f}{w:>7}{b:>8.0f}{wo:>8.0f}")
